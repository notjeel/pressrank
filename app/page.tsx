...
export default async function HomePage() {
  ...
  try {
    ...
    const { data: { user: u } } = await serverSupabase.auth.getUser();
    user = u;
  } catch (e) {
    // Graceful fallback for static/server build issues
  }
        ...
        >
          {user ? (
            <>
              ...
              >
                You are Signed In & Ready to Vote!
              </h2>
              <p style={{ fontSize: 14.5, color: "var(--muted)", maxWidth: "50ch", margin: "0 auto 30px" }}>
                Head over to the Arena to start reviewing statements, or explore the Leaderboard to see how channels rank.
              </p>
